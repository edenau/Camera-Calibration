...
%RansacHomog
% Runs a Ransac estimation of the homography
...
%
% Homog is the homography that has been identified.
% If no homography can be computed, then a 3x3 zero matrix is returned.
%
...
for Runs = 1:NRuns
    ...
    while RankTest == 1
        ...
        for j = 1:4
            ...
            r1 = j*2-1;
            r2 = j*2;
            [Regressor(r1:r2,:),DataVec(r1:r2)] = HomogRowPair(Correspond(:,SamplePoints(j)));
        ...
        if rank(Regressor) > 7
            ...
            % itself
            nCurrent = 0;
            CurrentConsensus = zeros(1,n);Error = zeros(n,1);
            ...
            for j = 1:n
                ...
                if ThisError < MaxError
                    ...
                    Error(j,1) = ... % error computation
        norm(HomogenousPoint(1:2) - [Correspond(1,j);Correspond(2,j)]);
...
if nBest > 0
    ...
    for j = 1:nBest
        ...
        [Regressor(r1:r2,:),DataVec(r1:r2)] = HomogRowPair(Correspond(:,BestConsensus(j)));
    end
