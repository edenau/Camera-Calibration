...
%FillImage 
% Takes in a Calibration grid definition and positions the
...
% Compute the positions of the GridCorners in the world coordinates
...
while InsideImage == 1
    ...
    % introduces randomness so we have a range of angles between the grid's
    ...
    Corners = zeros(2,4);
    for j = 1:4
        Corners(1:2,j) = HomogeneousCorners(1:2,j) / HomogeneousCorners(3,j);
    ...
    % the image. The next time around we have a better chance of achieving
    % our objective;
    if InsideImage == 1
