...
% The grid is a set of 4-element vectos [x y 0 1]'.
GridWidth = 1000;
GridIncrement = 10;
...
for CalImage = 1:nImages
    ...
    while Estimating == 1
        ...
        if Homog(3,3) > 0
            % This image worked. So record the homography and the consensus
            % set
...
D = diag(D);
[M,I] = min(D);
% K is the estimate of the kernal
K = V(:,I);
...
% Add in the symmetric componenets
Phi(2,1) = Phi(1,2);
Phi(3,1) = Phi(1,3);
Phi(3,2) = Phi(2,3);

% Check if the matrix is positive deinite
e = eig(Phi);
for j = 1:3
    if e(j) <= 0
