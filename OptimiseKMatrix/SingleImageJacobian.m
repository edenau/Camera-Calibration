function [NKMATJACOB, NFRAMEJACOB] = SingleImageJacobian( KMatrix ,NANGLE,... 
...
perturbation = 1.2; % 20% of perturbation
for a = 1 : n
    ...
    ROTANG = norm(NANGLE)-4*pi;
    ROTAXIS = NANGLE/norm(NANGLE);
    R = Rodrigues(ROTAXIS, ROTANG);
    ...
    k3 = KMatrix;
    k3(1,3) = KMatrix(1,3)*perturbation;
    k4 = KMatrix; 
    k4(2,2) = KMatrix(2,2)*perturbation;
    k5 = KMatrix;
    k5(2,3) = KMatrix(2,3)*perturbation;
    ...
    dk3 = (fk3(1:2)/fk3(3)-UVest)/(KMatrix(1,3)*(perturbation-1));
    dk4 = (fk4(1:2)/fk4(3)-UVest)/(KMatrix(2,2)*(perturbation-1));
    dk5 = (fk5(1:2)/fk5(3)-UVest)/(KMatrix(2,3)*(perturbation-1));
    
    NKMATJACOB(2*a-1:2*a,:) = [ dk1 dk2 dk3 dk4 dk5 ];
    
    for g = 1:3
        A = NANGLE;
        A(g) = NANGLE(g)*perturbation;
        ...
        dA = (fk(1:2)/fk(3) - UVest)/(NANGLE(g)*(perturbation-1));
    ...
    for t = 1:3
        T = NTRANSLATION;
        T(t) = NTRANSLATION(t)*perturbation;
        ...
        dT = (fk(1:2)/fk(3) - UVest)/ (NTRANSLATION(t)*(perturbation-1));
